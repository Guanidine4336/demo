...
// making it a directed graph as shortest path algos works on DAG
...
vector<int> dijkstra(int V, vector<vector<int>>& adjMatrix, int S) {
    ...
    while (!pq.empty()) {
        ...
        for (int adjnode = 0; adjnode < V; ++adjnode) {
            int edgewt = adjMatrix[node][adjnode];
            if (edgewt != 0 && dis + edgewt < dist[adjnode]) {
...
int main() {
    ...
    // checking the accuracy with DIJKSTRA ALGORITHM
    //  (nodes,adjmat,src)
    // ab iss src se sabki distance check krte fir floyd algorithm se compare krenge...
    vector<int> distance = dijkstra(25, timeMatrix, 0);
    ...
    // checking wrt cost like we did at line 218
    vector<int> cost = dijkstra(25, costMatrix, 0);
    for(auto it :cost){
        cout << it << " ";
    }
    cout << endl;
